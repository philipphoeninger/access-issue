...
Zweistufiger Flow, kein Einzelseiten-Layout. Der Flow macht sichtbar, wie sich Barrieren **kumulieren**: In Schritt 1 sind sie ärgerlich, in Schritt 2 führen sie zum Abbruch.

**Schritt 1 — Stellenanzeige**

| Barriere | Betroffene Nutzergruppen | Normbezug (vorläufig) |
| --- | --- | --- |
| Stellenausschreibung nur als barrierebehaftetes PDF zum Download | blinde und sehbehinderte Personen, motorische Einschränkungen, mobile Nutzung | WCAG 1.3.1, 1.3.2, 4.1.2 |
| Stellenbeschreibung in zu komplexer Sprache | kognitive Einschränkungen, Lernbehinderungen, Nicht-Muttersprachler, geringe Literalität | WCAG 3.1.5 |

**Schritt 2 — Bewerbungsformular**

| Barriere | Betroffene Nutzergruppen | Normbezug (vorläufig) |
| --- | --- | --- |
| Fehlende Labels an den Formularfeldern | blinde und sehbehinderte Personen, Spracheingabe-Nutzer | WCAG 1.3.1, 3.3.2, 4.1.2 |
| Keine Tastaturbedienung | motorische Einschränkungen, Screenreader-Nutzer, temporäre Einschränkungen | WCAG 2.1.1, 2.1.2 |
| Kein Fehler-Feedback bei ungültiger Eingabe | alle Gruppen, besonders kognitive Einschränkungen und Screenreader-Nutzer | WCAG 3.3.1, 3.3.3 |

Eine dritte Stufe (Terminbuchung/Bestätigung) ist bewusst **nicht** vorgesehen — sie hätte den Implementierungsaufwand erhöht, ohne zusätzlichen Lerngewinn zu bringen.

### 6.2 CSR-Kampagne (Priorität 2)

Kampagnen-Landingpage. Bewusst multimedialer als der Bewerbungsprozess — daher **mehr Barrieren (5–6 statt 5)**. Diese Asymmetrie ist gewollt: CSR-Landingpages sind in der Realität medienreicher als Bewerbungsformulare, und die Barrieren sind so gewählt, dass sie sich **nicht** mit Szenario 1 überschneiden.

| Barriere | Betroffene Nutzergruppen | Normbezug (vorläufig) |
| --- | --- | --- |
| Kampagnenvideo ohne Untertitel **und** ohne Transkript/Audiodeskription (*kombinierte Barriere*) | gehörlose und schwerhörige Personen, blinde Personen, situativ (laute/stille Umgebung) | WCAG 1.2.2, 1.2.3, 1.2.5 |
| Spenden-Fortschrittsbalken als reine Grafik ohne Textalternative | blinde und sehbehinderte Personen | WCAG 1.1.1, 4.1.2 |
| Social-Media-Einbettung ohne Alt-Texte, Text-Overlay mit schlechtem Kontrast | blinde und sehbehinderte Personen, Farbfehlsichtige, ältere Nutzer | WCAG 1.1.1, 1.4.3, 1.4.11 |
| Countdown ohne geeignete Live-Region | blinde und sehbehinderte Personen, kognitive Einschränkungen | WCAG 4.1.3, 2.2.1 |
| Spendenformular mit reinem Slider ohne Zahleneingabe und ohne Tastatur-Äquivalent | motorische Einschränkungen, Tastatur- und Screenreader-Nutzer, Tremor | WCAG 2.1.1, 2.5.7 |
| Automatisch scrollendes Testimonial-Karussell ohne Pause-Steuerung | kognitive Einschränkungen, Aufmerksamkeitsstörungen, vestibuläre Störungen | WCAG 2.2.2 |

### 6.3 Softwarebeschaffung (Priorität 3)

Inhaltlich noch in Klärung beim WERTE.IT-Team. Wird **zuletzt** umgesetzt. Die Architektur muss dieses Szenario aufnehmen können, ohne dass sein Zuschnitt heute feststeht.

**Hinweis zu den Normbezügen.** Die Angaben in beiden Tabellen sind vorläufig und vom
WERTE.IT-Team fachlich zu bestätigen. Ein Punkt ist bereits korrigiert: Der Spenden-Slider
war zunächst 2.5.1 („Zeigergesten") zugeordnet, das aber pfadbasierte Gesten wie Wischen
meint. Einschlägig ist **2.5.7 „Ziehbewegungen"**, neu in WCAG 2.2 — was per Ziehen
bedienbar ist, muss auch ohne Ziehen bedienbar sein. Da das Modul Unternehmen die
Rechtslage erklärt, ist ein falscher Paragraf ein inhaltlicher Fehler, kein Formalismus.
Die übrigen Bezüge sollten bei der Redaktion ebenfalls gegen 2.2 geprüft werden, nicht
gegen 2.1.

### 6.4 Kombinierte Barrieren

Mindestens eine Barriere besteht aus gekoppelten Teilaspekten, bei denen das Beheben nur eines Teils das Problem **nicht** vollständig löst (Referenzfall: Kampagnenvideo ohne Untertitel und ohne Transkript). Didaktisches Ziel: Barrierefreiheit ist keine Liste unabhängiger Häkchen.
...
- Zweistufiger Flow mit den fünf in Abschnitt 6.1 genannten Barrieren.
...
- Landingpage mit den in Abschnitt 6.2 genannten Barrieren, darunter mindestens eine kombinierte Barriere.
...
**G. Barrierefreiheit des Tools selbst**
...
- **Szenario Softwarebeschaffung** — abhängig von der inhaltlichen Klärung durch WERTE.IT.
...
| Frage | Wer entscheidet |
| --- | --- |
| Inhaltlicher Zuschnitt des Szenarios Softwarebeschaffung | WERTE.IT-Team |
...
| --- | --- | --- |
| 1 | Grundgerüst, Rahmen-Architektur, Panel, State-in-URL, Szenario Bewerbungsprozess | Fachtexte für Szenario 1 |
| 2 | Szenario CSR-Kampagne inklusive kombinierter Barriere | Fachtexte, ggf. Videomaterial |
| 3 | Szenario Softwarebeschaffung | inhaltliche Klärung durch WERTE.IT |
